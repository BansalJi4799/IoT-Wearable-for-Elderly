...
AltSoftSerial altSerial(8, 7); // Create an instance of AltSoftSerial
...
void setup()
{
  ...
  Serial1.begin(9600);
  altSerial.begin(9600); // Connect Serial
...
void loop()
{
  while (Serial1.available())
  {
    if (gps.encode(Serial1.read()))
    {
      ...
      logDataToSD(latitude, longitude, distance, speed, acceleration, espData);
...
void sendSDCardData()
{
  ...
  if (dataFile)
  {
    while (dataFile.available() && altSerial.available())
    {
      char dataChar = dataFile.read();
      receivedData += dataChar;
      altSerial.println(dataChar);
    }
    dataFile.close();
  }
}
...
void readESPData(String &receivedData)
{
...
void logDataToSD(float latitude, float longitude, float distance, float speed, float acceleration, String espData)
{
  // Open the text file
  dataFile = SD.open("datalog.txt", FILE_WRITE);
